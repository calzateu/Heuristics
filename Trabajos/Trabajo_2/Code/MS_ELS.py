...
def MS_ELS(utils, problem_information, dist_matrix, demands, max_capacity, max_distance, num_cars, num_insertions, num_relocations, num_mutations, ni=5, nc=2, nsol=5, std=0.1, max_iterations=200):
    ...
    for h in range(nsol):
        ...
        if distance_exceed(traveled_distances_S, max_distance, num_cars) < f_asteristo:
            s_asterisco = S.copy()
            traveled_distances_asterisco = traveled_distances_S.copy()
            f_asteristo = distance_exceed(traveled_distances_S, max_distance, num_cars)
        ...
        S_prima, traveled_distances_prima = ELS(utils, problem_information, dist_matrix,
            demands, max_capacity, max_distance, num_cars, ni, nc, generate_initial_solution=False,
        ...
        if distance_exceed(traveled_distances_prima, max_distance, num_cars) < f_asteristo:
